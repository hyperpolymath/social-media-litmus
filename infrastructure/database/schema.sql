-- NUJ Social Media Ethics Monitor - Database Schema
...
-- Policy snapshots (TimescaleDB hypertable for time-series data)
...
SELECT create_hypertable('policy_snapshots', 'captured_at',
...
SELECT create_hypertable('policy_changes', 'detected_at',
    chunk_time_interval => INTERVAL '1 month',
    if_not_exists => TRUE
);
...
COMMENT ON TABLE policy_snapshots IS 'TimescaleDB hypertable storing point-in-time policy content';
